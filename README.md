...
Setup Overview
The goal of this setup is to create a real-time data warehouse that ingests data from a PostgreSQL database using Debezium, processes it with Apache Flink, and stores it in different layers using Apache Hudi. Finally, it aggregates data and visualizes it using Kibana.
...
Docker Compose
The setup is orchestrated using Docker Compose, which allows you to define and run multi-container applications easily.
...
Postgres
Start a PostgreSQL client to interact with the PostgreSQL source tables.
...
Flink Table Connectors
Flink Packages - Connectors
Flink Faker
...
DWD (Data Warehouse - Data)
Create a 'dwd' (Data Warehouse - Data) database.
...
DWB (Data Warehouse - Business)
Create a 'dwb' (Data Warehouse - Business) database.
...
Kibana Dashboard
Create a Kibana dashboard to visualize the aggregated data in Elasticsearch.
